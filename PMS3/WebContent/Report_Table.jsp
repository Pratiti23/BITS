...
{  
	...
	if(xhttp.readyState===4 && xhttp.status===200)
 	...
 	//also pass session value
	xhttp.open("POST","SearchTaskServlet?valajax="+string_value,true);
	xhttp.send();
...
<div class="topnav">
 <span>Hello <%if(session.getAttribute("name")!=null && (session.getAttribute("type").equals("Technical Consultant")||session.getAttribute("type").equals("admin")))
{
%>
${name}</span>
...
<br>
<center><span style="color:white; font-weight: bold;">Project Name</span>&nbsp;
<select name="pn" required>
<option value="">Select</option>
<% Model m = new Model();
String projectName="select project_name from xxevo_pm_project_tbl order by project_name";
String reportName="select report_name from xxevo_pm_task_dtl_tbl";
String taskNum="select Task_number from xxevo_pm_task_dtl_tbl";
if(session.getAttribute("type").equals("Technical Consultant"))
{
	reportName="select report_name from xxevo_pm_task_dtl_tbl where created_by="+(String)session.getAttribute("email")+"or technical_owner="+(String)session.getAttribute("email");
	taskNum="select Task_number from xxevo_pm_task_dtl_tbl where created_by="+(String)session.getAttribute("email")+"or technical_owner="+(String)session.getAttribute("email");
}
ResultSet rs2=Dao.validate(projectName,m);
while(rs2.next()){%>
    <option value="<%=rs2.getString(1) %>"> <%=rs2.getString(1) %></option>
  <% }%>
    </select>
&nbsp;
<span style="color:white; font-weight: bold;">Task Number</span>&nbsp;
<select name="tn" required>
<option value="">Select</option>
<%rs2=Dao.validate(taskNum,m);
while(rs2.next()){%>
    <option value="<%=rs2.getString(1) %>"> <%=rs2.getString(1) %></option>
  <% }%>
    </select>&nbsp;
<span style="color:white; font-weight: bold;">Report Name</span>&nbsp;
<select name="rn" required>
<option value="">Select</option>
<%rs2=Dao.validate(reportName,m);
while(rs2.next()){%>
    <option value="<%=rs2.getString(1) %>"> <%=rs2.getString(1) %></option>
  <% }%>
    </select>
...
<div class="limiter">
					...
					<thead>
							<tr class="row100 head" >
								...
								Connection con=Dao.connect();
								 CallableStatement stmt =con.prepareCall("BEGIN DisplayAllTask(?,?); END; ");
								
								   stmt.setInt(1, Integer.parseInt((String)session.getAttribute("email")));
   ...
   { %>
							<tr class="row100">
								<td class="column100 column9" data-column="column9" style="font-weight: normal;"><a href="Report_update.jsp?msg=<%=rs.getString(20)%>">Edit</a></td>
